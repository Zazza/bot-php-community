...
// Заголовок общий с обычным выпуском (digestTitle), но под ним — подпись-предупреждение
// (fakeDisclaimer): статьи стали слишком правдоподобны, читатель принимал шутку всерьёз.
// Ссылки не проходят модерацию реальных фидов — у выпуска свой white-список доменов
// (виды «правдоподобных» ссылок) и своя санитайз-логика по блокам.
const fakePostMaxUTF16 = 3900

// fakeDisclaimer — подпись-предупреждение отдельной строкой под шапкой. Обычный текст,
// не markdown: md.ToHTML курсив/подчёркивание не рендерит, эмодзи 🎭 отделяет её от
// жирного заголовка. Добавляется в коде (assembleFakePost), НЕ через LLM — содержит
// слова из fakeMetaMarkers, которые санитайзер вырезает из тела: о выдуманности вправе
// говорить только шапка, статьи остаются deadpan.
const fakeDisclaimer = "🎭 Пятничный выпуск — всё выдумано, не всерьёз."
...
// fakeMetaMarkers — МЕТА-маркеры выдуманности (не контентные слова юмора): о шуточности
// выпуска говорит ровно одна контролируемая кодом строка (fakeDisclaimer); их утечка из
// LLM засорила бы тело вторыми дисклеймерами и сломала deadpan статей. Страховка: тело
// генерит промпт, эти слова из него вырезаются — дублировать подпись в статьях нельзя.
...
// fakeHeader — шапка пятничного выпуска: общий с обычным дайджестом заголовок + подпись
// на отдельной строке.
func fakeHeader() string { return digestTitle + "\n" + fakeDisclaimer }

// fakeBodyBudget — бюджет UTF-16 на тело поста (лимит TG минус шапка с подписью и
// разделитель "\n\n").
func fakeBodyBudget() int { return fakePostMaxUTF16 - md.UTF16Len(fakeHeader()) - 2 }

// assembleFakePost собирает пост: заголовок обычного дайджеста + подпись-предупреждение
// + тело в пределах лимита TG.
